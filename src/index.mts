...
import { createEstateClient, ChatServer, ChatEntry, ChatMessage } from "chat-service";

const estate = createEstateClient(new ServiceOptions(false, false));

const chatServer = estate.getWorker(ChatServer, "Scott's No-kill Animal Shelter");
...
if (args[0] === "--console") {
    ...
    await estate.subscribeMessageAsync(chatServer, ChatMessage, (chatMessage: ChatMessage) => {
        outputChatEntry(chatMessage.entry);
    });
} else {
